      ...
      <div className="hero bg-primary text-primary-content py-16 md:py-20">
        <div className="hero-content text-center">
          <div className="max-w-4xl">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 md:mb-6">Kits de Robotica</h1>
            <p className="text-lg md:text-xl">
              Explora nuestra seleccion de kits de robotica especialmente disenados para cada nivel de aprendizaje
            </p>
          </div>
        </div>
      </div>
            ...
            {products.map((product) => (
              <div key={product.id} className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow">
                ...
                <div className="card-body p-4 md:p-6">
                  <h2 className="card-title text-xl md:text-2xl">{product.title}</h2>
                  ...
                  {product.features && product.features.length > 0 && (
                    <div className="mt-4 space-y-2">
                      {product.features.slice(0, 3).map((feature) => (
                        <div key={feature.id} className="border-l-2 border-primary pl-3">
                          <h3 className="font-semibold text-sm md:text-base">{feature.subtitle}</h3>
                  ...
                  <div className="card-actions justify-end mt-4">
                    <Link to={`/productos/${product.id}`} className="btn btn-primary w-full sm:w-auto">
                      Ver Detalles
                    </Link>
                  </div>
                </div>
      ...
      {products.length > 0 && (
        <div className="bg-base-100 py-12 md:py-16">
          <div className="container mx-auto px-4 text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">¿Necesitas ayuda para elegir?</h2>
            <p className="text-base md:text-lg text-base-content/70 mb-6 max-w-2xl mx-auto">
              Nuestro equipo esta listo para ayudarte a encontrar el kit perfecto para tus necesidades
            </p>
